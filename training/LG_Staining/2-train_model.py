...
# %%
dataset_dir = 'datasets_preprocessed/LG_Staining'
...
imags_train, masks_train, filenames = get_data_train(dataset_dir, img_rows, img_cols, aug_len)
...
if os.path.exists(os.path.join(weight_dir, 'LG_Staining_model.h5')):
    model.load_weights(os.path.join(weight_dir, 'LG_Staining_model.h5'))
    print('\n Weights loaded successfully \n')

model_checkpoint = ModelCheckpoint(
    os.path.join(weight_dir, 'LG_Staining_model.h5'), 
    monitor='val_loss', 
    save_best_only=True  
    )

csv_logger = CSVLogger(
    os.path.join(log_dir,  'LG_Staining_log.txt'), 
    separator=',', 
    append=False)
